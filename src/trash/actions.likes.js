...
export const getInitLikeState = (authId, uploadId) => dispatch => {
  return new Promise((resolve, reject) => {
    return firebase.database().ref(`uploads/${uploadId}/likes`).once('value', (snapshot) => {
...
export const addLike = (authId, uploadId, publisherId) => dispatch => {
  ...
  firebase.database().ref(`/featured/${uploadId}`).once('value', snapshot => {
    let exist = !!snapshot.val();
    if(exist){
      firebase.database().ref(`/featured/${uploadId}/likes/${authId}/id`).set(authId)
    }
  })
};
...
export const removeLike = (authId, uploadId, publisherId) => dispatch => {
  ...
  firebase.database().ref(`/featured/${uploadId}`).once('value', snapshot => {
    let exist = !!snapshot.val();
    if(exist){
      firebase.database().ref(`/featured/${uploadId}/likes/${authId}`).remove();
    }
  })
};